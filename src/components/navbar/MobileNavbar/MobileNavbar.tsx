...
const Navbar = ({ isMenuOpen, setIsMenuOpen }: MobileNavbarProps) => {
  ...
  const isActive = (anchor: string) => {
    if (anchor === '#') {
      // Home is active when on the root path
      return pathname === `/${locale}` || pathname === `/${locale}/`;
    } else if (anchor.startsWith('#')) {
      // Anchor links (like #about) are on the home page
      // Consider them active when on the home page
      return pathname === `/${locale}` || pathname === `/${locale}/`;
    } else {
      // Regular paths
  ...
  return (
    ...
    <AnimatePresence>
        {isMenuOpen && (
          ...
          >
            <motion.div
              className="bg-white/80 rounded-3xl p-6 w-[200px] shadow-lg border border-gray-200 pointer-events-auto"
                    ...
                    >
                      <motion.button 
                        ...
                        className={`w-full py-2 px-4 rounded-lg transition-colors ${
                          isActive(item.anchor) ? 'bg-black/5' : ''
                        }`}
                        whileHover={{ 
                ...
                {otherSubItems.map((sub, i) => (
                  ...
                  >
                    <motion.button 
                      ...
                      id={`${isActive(sub.anchor) ? 'font-gbold' : 'font-g-regular'}`}
                      className={`w-full py-2 px-4 rounded-lg transition-colors ${
                        isActive(sub.anchor) ? 'bg-black/5' : ''
                      }`}
                      whileHover={{ 
                        backgroundColor: "rgba(0,0,0,0.06)", 
                        scale: 1.02 
    ...
    <nav className="bg-white/90 backdrop-blur-md outline-none outline-1 outline-black/60 flex justify-center items-center outline-offset-0 rounded-full p-4 fixed bottom-[20px] left-1/2 transform -translate-x-1/2 w-[100px] h-[40px] select-none z-50">
    <button
