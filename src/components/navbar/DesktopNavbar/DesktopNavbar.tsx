...
export default function Navbar() {
    ...
    return (
        <div>
            <div className="relative flex items-center bg-white/80 px-2 h-[48px] rounded-full shadow-[0px_1px_7px] shadow-gray-300/80 backdrop-blur-md">
                    ...
                    >
                        ...
                        <AnimatePresence>
                            {item.subItems.length > 0 && (hovered === index || isSubmenuOpen) && (
                                <motion.div
                                    id={`submenu-${index}`}
                                    className="absolute -left-[10px] mt-4 bg-white/90 rounded-2xl border shadow-[0px_1px_7px] backdrop-blur-md shadow-gray-300/80 py-1 min-w-[124px] z-50 overflow-hidden"
                                ...
                                >
                                    <motion.div className="py-1 px-1">
                                        {item.subItems.map((sub, subIndex) => (
                                            <motion.button
                                                key={sub.label}
                                                className="w-full text-left px-4 py-3 rounded-xl hover:bg-black/5 transition-all whitespace-nowrap flex items-center"
                                                onClick={(e) => {
                                                    e.preventDefault();
                                                    setHovered(null);
                                                    setIsSubmenuOpen(false);
